...
// Tu código aquí
$ventas = [
    ["producto_id" => 1, "cliente_id" => 101, "cantidad" => 2, "fecha" => "2025-09-10"],
    ["producto_id" => 2, "cliente_id" => 102, "cantidad" => 1, "fecha" => "2025-09-11"],
    ["producto_id" => 3, "cliente_id" => 101, "cantidad" => 3, "fecha" => "2025-09-12"],
    ["producto_id" => 1, "cliente_id" => 103, "cantidad" => 1, "fecha" => "2025-09-13"],
    ["producto_id" => 5, "cliente_id" => 102, "cantidad" => 2, "fecha" => "2025-09-14"],
    ["producto_id" => 3, "cliente_id" => 103, "cantidad" => 4, "fecha" => "2025-09-15"],
    ["producto_id" => 6, "cliente_id" => 101, "cantidad" => 1, "fecha" => "2025-09-16"]
];
...
function generarResumenVentas($ventas, $productos, $clientes) {
    ...
    $productoMasVendido = array_values(array_filter($productos, fn($p) => $p['id'] == $productoMasVendidoId))[0];

    // Obtener cliente que más compró
    $clienteMasCompradorId = array_search(max($ventasPorCliente), $ventasPorCliente);
    $clienteMasComprador = array_values(array_filter($clientes, fn($c) => $c['id'] == $clienteMasCompradorId))[0];

    // Mostrar informe
    echo "\n===== Resumen de Ventas =====\n";
    echo "Total de unidades vendidas: $totalVentas\n";
    echo "Producto más vendido: {$productoMasVendido['nombre']} ({$ventasPorProducto[$productoMasVendidoId]} unidades)\n";
    echo "Cliente que más compró: {$clienteMasComprador['nombre']} ({$ventasPorCliente[$clienteMasCompradorId]} unidades)\n";
}
